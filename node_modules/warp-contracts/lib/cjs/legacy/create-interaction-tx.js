...
exports.createDummyTx = exports.createInteractionTx = void 0;
...
async function createInteractionTx(arweave, signer, contractId, input, tags, target = '', winstonQty = '0', dummy = false, isTestnet, reward) {
    ...
    interactionTx.addTag(KnownTags_1.SMART_WEAVE_TAGS.APP_NAME, 'SmartWeaveAction');
    // use real SDK version here?
    interactionTx.addTag(KnownTags_1.SMART_WEAVE_TAGS.APP_VERSION, '0.3.0');
    interactionTx.addTag(KnownTags_1.SMART_WEAVE_TAGS.SDK, 'Warp');
    interactionTx.addTag(KnownTags_1.SMART_WEAVE_TAGS.CONTRACT_TX_ID, contractId);
    interactionTx.addTag(KnownTags_1.SMART_WEAVE_TAGS.INPUT, JSON.stringify(input));
    if (isTestnet) {
        interactionTx.addTag(KnownTags_1.WARP_TAGS.WARP_TESTNET, '1.0.0');
    }
    if (signer) {
