...
class HandlerBasedContract {
    ...
    async viewState(input, tags = [], transfer = CreateContract_1.emptyTransfer) {
        ...
        return await this.callContract(input, 'view', undefined, undefined, tags, transfer);
    }
    ...
    async bundleInteraction(input, options) {
        ...
        const response = this._warpFetchWrapper.fetch(`${(0, utils_1.stripTrailingSlash)(this._evaluationOptions.sequencerUrl)}/gateway/sequencer/register`, {
            method: 'POST',
    ...
    async createInteraction(input, tags, transfer, strict, bundle = false, vrf = false, reward) {
        ...
        if (vrf) {
            tags.push({
                name: KnownTags_1.WARP_TAGS.REQUEST_VRF,
                value: 'true'
            });
        }
    ...
    async fetchRemoteContractState(contractId) {
        return this._warpFetchWrapper
            ...
            .catch((error) => {
            var _a;
            throw new Error(`Unable to read contract state from DRE. ${error.status}. ${(_a = error.body) === null || _a === void 0 ? void 0 : _a.message}`);
        });
    }
